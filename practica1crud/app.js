const cargarPosts=async()=>{
    ...
    data.map(item=>{
        ...
        <td>${item.title}</td>
        <td>${item.body}</td>
        <td><button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#editModal" onclick="cargarPost(${item.id})"> <i class="bi bi-magic"></i> Editar</button></td>
        <td><button type="button" class="btn btn-secondary btn-sm" data-bs-toggle="modal" data-bs-target="#deleteModal" onclick="eliminarPost(${item.id})"><i class="bi bi-backspace m-2"></i>Eliminar</button></td>
        </tr>`;
    })
}
...
<th scope="row">${respuesta.id}</th>
        ...
        <td><button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#editModal"> <i class="bi bi-pencil-square"></i> Editar</button></td>
        <td><button type="button" class="btn btn-secondary btn-sm"  data-bs-toggle="modal" data-bs-target="#deleteModal"><i class="bi bi-backspace m-2"></i>Eliminar</button></td>
        `
